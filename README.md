...
Kaiden is a desktop application for working with AI model providers, MCP (Model Context Protocol) servers, and container/Kubernetes environments from one workspace. It is built on the [Podman Desktop](https://github.com/containers/podman-desktop) extension architecture and adds AI-specific registries, ACP sessions, and RAG tooling on top of it.
...
- **Podman Desktop** — Kaiden reuses Podman Desktop's main/renderer/preload architecture and extension system. The container and Kubernetes features come from that base; AI providers, MCP, and ACP sessions are added on top.
- **LM Studio, Open WebUI, and other chat front-ends** — those focus on local model inference and chat. Kaiden includes ACP sessions but also covers MCP and container/Kubernetes operations.
- **Goose** — Goose is an agent provider; Kaiden embeds it as one provider and adds a UI for managing model providers and MCP servers.

## Architecture

Kaiden is an Electron application written in TypeScript with a Svelte renderer. The codebase is a pnpm monorepo:

- `packages/main` — Node.js main process, plugin system, and registries (`ProviderRegistry`, `ContainerProviderRegistry`, `KubernetesClient`, `MCPManager`).
